import React, { useState } from 'react';
import { X, User, BadgeCheck, MapPin, Phone, GraduationCap, Image as ImageIcon, FileSignature, Loader2, Receipt } from 'lucide-react';
import AuthenticatedImage, { preloadImage } from '../AuthenticatedImage';
import { type ApplicantCard } from '../../types/card';
import { Suspense, lazy } from 'react';

const PaymentProofModal = lazy(() => import('../PaymentProofModal'));

interface Props {
  data: ApplicantCard;
  onClose: () => void;
}

const ApplicantDetailsModal: React.FC<Props> = ({ data, onClose }) => {
  const [viewingPaymentProof, setViewingPaymentProof] = useState<string | null>(null);

  return (
    <>
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-background/80 backdrop-blur-sm animate-in fade-in duration-300">
        {/* Modal Content */}
        <div className="relative bg-card rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-border">

          {/* Header */}
          <div className="flex justify-between items-center p-8 bg-muted border-b border-border">
            <div className="flex items-center gap-6">
              <div className="h-14 w-14 rounded-lg bg-primary flex items-center justify-center text-primary-foreground shadow-lg shadow-primary/20">
                <User size={28} />
              </div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] opacity-80">Student Details</span>
                  <span className="w-1 h-1 rounded-full bg-border" />
                  <span className="text-[10px] font-bold text-primary uppercase tracking-[0.2em]">{data.idNumber}</span>
                </div>
                <h2 className="text-3xl font-black text-foreground tracking-tighter uppercase leading-none">
                  {data.fullName}
                </h2>
              </div>
            </div>
            <button
              onClick={onClose}
              className="p-1.5 hover:bg-accent rounded-md transition-colors text-muted-foreground hover:text-foreground"
            >
              <X size={18} />
            </button>
          </div>

          {/* Body */}
          <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-card text-foreground">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">

              {/* Personal Information */}
              <div className="space-y-8">
                <div>
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-6 flex items-center gap-2">
                    <BadgeCheck size={14} className="text-primary" />
                    Personal Info
                  </h3>

                  <div className="space-y-6">
                    <div className="group">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Full Name</label>
                      <p className="text-lg font-black text-foreground uppercase tracking-tight">{data.fullName}</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">ID Number</label>
                        <p className="text-sm font-bold text-foreground font-mono tracking-wider">{data.idNumber}</p>
                      </div>
                      <div>
                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Course</label>
                        <p className="text-sm font-bold text-foreground uppercase">{data.course}</p>
                      </div>
                    </div>

                    <div>
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Address</label>
                      <div className="flex gap-2 items-start mt-1">
                        <MapPin size={14} className="text-muted-foreground/40 mt-0.5 shrink-0" />
                        <p className="text-sm font-medium text-muted-foreground line-height-relaxed">{data.address || 'No address provided'}</p>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="pt-8 border-t border-border">
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-6 flex items-center gap-2">
                    <Phone size={14} className="text-primary" />
                    Emergency Contact
                  </h3>
                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Guardian / Parent</label>
                      <p className="text-sm font-bold text-foreground uppercase">{data.guardian_name || 'Unset'}</p>
                    </div>

                    <div>
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Phone Number</label>
                      <p className="text-sm font-bold text-primary font-mono bg-primary/5 inline-block px-2 py-0.5 rounded-md">{data.guardian_contact || 'None'}</p>
                    </div>
                  </div>
                </div>

                {data.paymentProof && (
                  <div className="pt-8 border-t border-border">
                      <button
                        onClick={() => setViewingPaymentProof(data.paymentProof || null)}
                        onMouseEnter={() => {
                          if (data.paymentProof && !data.paymentProof.toLowerCase().endsWith('.pdf')) {
                            preloadImage(data.paymentProof);
                          }
                        }}
                        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-bold text-[11px] uppercase transition-all"
                      >
                        <Receipt size={14} /> View Payment Proof
                      </button>
                  </div>
                )}
              </div>

              {/* Photo and Signature */}
              <div className="space-y-8">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-6 flex items-center gap-2">
                  <ImageIcon size={14} className="text-primary" />
                  Photo & Signature
                </h3>

                {/* ID Photo */}
                <div className="group">
                  <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-3 block">
                    ID Photo
                  </label>
                  <div className="relative w-full aspect-[4/5] bg-secondary/30 rounded-lg overflow-hidden border-2 border-border shadow-inner group-hover:border-primary/20 transition-all duration-500">
                    {!data.photo ? (
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground/30 bg-muted">
                        <User size={64} className="mb-4 opacity-50" />
                        <p className="text-[10px] font-black uppercase tracking-widest">No Photo</p>
                      </div>
                    ) : (
                      <AuthenticatedImage
                        src={data.photo}
                        alt={`${data.fullName}'s photo`}
                        className="w-full h-full object-cover transition-all duration-700 group-hover:scale-105"
                      />
                    )}
                  </div>
                </div>

                {/* Signature */}
                <div className="group">
                  <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-3 block">
                    Signature
                  </label>
                  <div className="relative w-full h-32 bg-secondary/30 rounded-lg overflow-hidden border-2 border-border shadow-inner p-4 group-hover:border-primary/20 transition-all duration-500">
                    {!data.signature ? (
                      <div className="absolute inset-0 flex items-center justify-center text-muted-foreground/30 bg-muted">
                        <FileSignature size={24} className="mr-2 opacity-50" />
                        <p className="text-[10px] font-black uppercase tracking-widest">No Signature</p>
                      </div>
                    ) : (
                      <AuthenticatedImage
                        src={data.signature}
                        alt={`${data.fullName}'s signature`}
                        className="w-full h-full object-contain transition-all duration-700"
                      />
                    )}
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <style>{`
        .line-height-relaxed {
          line-height: 1.6;
        }
      `}</style>
      </div>

      {viewingPaymentProof && (
        <Suspense fallback={<div className="fixed inset-0 bg-black/40 z-[70] flex items-center justify-center"><Loader2 className="animate-spin text-white" size={48} /></div>}>
          <PaymentProofModal
            url={viewingPaymentProof}
            onClose={() => setViewingPaymentProof(null)}
          />
        </Suspense>
      )}
    </>
  );
};

export default ApplicantDetailsModal;